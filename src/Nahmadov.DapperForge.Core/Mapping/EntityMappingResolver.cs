...
internal static class EntityMappingResolver
{
        ...
        IReadOnlyCollection<PropertyInfo> keyProps)
    {
        ...
        foreach (var prop in snapshot.ScalarProperties)
        {
            ...
            var columnName = !string.IsNullOrWhiteSpace(propConfig?.ColumnName)
                ? propConfig!.ColumnName
                : attrSnapshot?.Column?.Name ?? prop.Name;

            var columnAttribute = string.Equals(columnName, prop.Name, StringComparison.Ordinal)
                ? null
                : new ColumnAttribute(columnName);

            var generatedAttribute = attrSnapshot?.Generated;
            var isKey = keyLookup.Contains(prop);

            if (propConfig?.IsAutoGenerated is bool autoGeneratedOverride)
            {
                generatedAttribute = autoGeneratedOverride
                    ? generatedAttribute ?? new DatabaseGeneratedAttribute(DatabaseGeneratedOption.Identity)
                    : null;
            }
            else if (config.HasKey &&
                     isKey &&
                     keyLookup.Count == 1 &&
                     generatedAttribute is null &&
